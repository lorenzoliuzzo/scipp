...
namespace scipp::geometry {
    ...
    template <size_t DIM, typename VECTOR_TYPE>
    ...
    template <size_t DIM, typename SEGMENT_TYPE>
    ...
    template <size_t DIM, typename CIRCLE_TYPE>
    ...
    template <size_t DIM, typename SPHERE_TYPE>
    ...
    static constexpr matrix<typename SPHERE_TYPE::point_type, DIM> linspace(const SPHERE_TYPE& other) noexcept {
        ...
        // for (size_t i{}; i < static_cast<unsigned int>(DIM / 10); ++i)
        //     for (size_t j{}; j < static_cast<unsigned int>(DIM / 10); ++j)
    ...
    //     // for (size_t m{}; m < M_theta - static_cast<physics::scalar_m>(1.0); ++m) {

    //     for (size_t m{}; m < M_theta; ++m) {

    //         theta = math::constants::pi * (m + 0.5) / M_theta;
    //         M_phi = std::round(2 * math::constants::pi * math::op::sin(theta) / d_phi);

    //         for (size_t n{}; n < (M_phi - 1); ++n) {
